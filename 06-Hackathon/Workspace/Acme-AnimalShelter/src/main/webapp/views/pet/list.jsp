...
<display:table pagesize="5" class="displaytag" keepStatus="true"
		...
		<jstl:if test="${viewForDelete == true}">
			<display:column>
				<form name="submitForm" method="POST"
					action="pet/administrator/delete.do?petId=${row.id }">
	...
	<display:column property="date" title="${momentHeader}"
	...
	<security:authorize access="hasRole('EMPLOYEE')">
		<display:column>
			<acme:links url="center/employee/list.do?petId=${row.id }"
				code="pet.center" />
		</display:column>
	</security:authorize>

	<security:authorize access="hasRole('CLIENT')">
	<jstl:if test="${row.status  == true}">
		<display:column>
			<acme:links url="application/client/create.do?petId=${row.id }"
				code="pet.application" />
		</display:column>
	</jstl:if>
	</security:authorize>
	...
	<security:authorize access="hasRole('VETERINARY')">
		<display:column>
			<jstl:if test="${row.medicalReport == null}">
				<acme:links url="medicalReport/veterinary/create.do?petId=${row.id}"
					code="pet.createMedicalReport" />
			</jstl:if>
			<jstl:if test="${row.medicalReport.veterinary==veterinaryPrincipal}">
				<acme:links
					url="medicalReport/veterinary/edit.do?medicalReportId=${row.medicalReport.id}"
