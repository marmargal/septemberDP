...
public class CountryAdministratorController extends AbstractController {
	...
	public ModelAndView save(@Valid final CountryForm countryForm,
		...
		if (binding.hasErrors()) {
			res = this.createEditModelAndView(countryForm,
					"country.params.error");
		} else
			try {

				Country country = this.countryService.reconstruct(countryForm,
						binding);
	...
	public ModelAndView delete(CountryForm countryForm, BindingResult binding) {
		...
		try {
			...
			Country country = this.countryService.reconstruct(countryForm,
					binding);
			if (!country.getLaw().isEmpty()) {
				res = this.createEditModelAndView(countryForm,
						"country.commit.error.law");
	...
	protected ModelAndView createEditModelAndView(
			final CountryForm countryForm, final String message) {
		ModelAndView res;
